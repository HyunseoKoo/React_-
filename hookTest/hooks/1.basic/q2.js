...
function Q2() {
  ...
  const onSaveText = (e) => {
    setInputText(e.target.value); // inputText 변수에 사용자가 입력한 값 저장
  ...
  */
  const onChangeColor = () => {
    ...
    } else HtmlRef.current.style.color = 'green';
  };
  ...
  return (
    <>
      ...
      <div>
        <h2>문제 2-1</h2>
        <p>
          <input onChange={onSaveText} />
        </p>
        <p>
          <button onClick={onAddList}>추가</button>
        </p>
        <p>
          <button onClick={onSubmitArr}>제출</button>
        </p>
